...
class StripePlAdmin extends Process implements Module, ConfigurableModule {
	...
	protected function renderTable(array $purchases, array $columns): string {
		...
		if (!empty($sums)) {
			...
			foreach ($columns as $col) {
				if ($firstCol) {
					$totalLabel = '<strong>' . $this->_('Total') . '</strong>';
					$tooltipText = htmlspecialchars($this->_('Transaction view: Sum of all complete checkout sessions (full cart value)'));
					$totalLabel .= " <i class='fa fa-info-circle' style='opacity:0.5;font-size:0.9em;cursor:help' title=\"{$tooltipText}\" uk-tooltip></i>";
					$summaryRow[] = $totalLabel;
					$firstCol = false;
	...
	protected function renderTabs(string $active): string {
		...
		$tabs = [
			'purchases' => [
				'url' => $baseUrl,
				'label' => 'Purchases',
				'tooltip' => 'Transaction view: Shows complete checkout sessions. Total = Full transaction value including all products purchased together.'
			],
			'products' => [
				'url' => $baseUrl . 'products/',
				'label' => 'Products',
				'tooltip' => 'Product performance view: Shows product-specific metrics. Total = Revenue generated by each individual product.'
			],
			'customers' => [
				'url' => $baseUrl . 'customers/',
				'label' => 'Customers',
				'tooltip' => 'Customer overview: Shows customer lifetime value and purchase history.'
			],
		];
		...
		foreach ($tabs as $key => $tab) {
			...
			$tooltip = isset($tab['tooltip']) ? " title=\"" . htmlspecialchars($tab['tooltip']) . "\" uk-tooltip" : "";
			$icon = isset($tab['tooltip']) ? " <i class='fa fa-info-circle' style='opacity:0.5;font-size:0.9em'></i>" : "";
			$out .= "<li class='{$liClass}' role='presentation'><a href='{$tab['url']}' class='{$aClass}' role='tab'{$tooltip}>{$tab['label']}{$icon}</a></li>";
		}
		$out .= "</ul>";
		...
		if (empty($productData)) {
			...
			if (!empty($sums)) {
				...
				foreach ($columns as $col) {
					if ($firstCol) {
						$totalLabel = '<strong>' . $this->_('Total') . '</strong>';
						$tooltipText = htmlspecialchars($this->_('Product performance view: Sum of revenue generated by each individual product'));
						$totalLabel .= " <i class='fa fa-info-circle' style='opacity:0.5;font-size:0.9em;cursor:help' title=\"{$tooltipText}\" uk-tooltip></i>";
						$summaryRow[] = $totalLabel;
						$firstCol = false;
		...
		if (empty($customerData)) {
			...
			if (!empty($sums)) {
				...
				foreach ($columns as $col) {
					if ($firstCol) {
						$totalLabel = '<strong>' . $this->_('Total') . '</strong>';
						$tooltipText = htmlspecialchars($this->_('Customer overview: Sum of all customer lifetime values'));
						$totalLabel .= " <i class='fa fa-info-circle' style='opacity:0.5;font-size:0.9em;cursor:help' title=\"{$tooltipText}\" uk-tooltip></i>";
						$summaryRow[] = $totalLabel;
						$firstCol = false;
					} elseif (isset($sums[$col])) {
